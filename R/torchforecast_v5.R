...
# Version 5.0  (single file)
# ================================================================
#
# KEY ADDITIONS v5.0
# ────────────────────────────────────────────────────────────────
#   ① forecast / modeltime integration
#       All ARIMA, ETS, Prophet, ARFIMA, GARCH, HAR-RV models are
#       wrapped inside a unified modeltime table:
#         model_table <- tf_modeltime_table(result1, result2, ...)
#         calibrated  <- modeltime_calibrate(model_table, test_df)
#         accuracy    <- modeltime_accuracy(calibrated)
#         future_fc   <- modeltime_forecast(calibrated, new_data)
#
#   ② Redesigned predict() — three call signatures for ALL models:
#       # 1. Future horizon (no new data)
#       predict(model, horizon = 12L)
#
#       # 2. Apply to new/test data rows (each row = 1 time step)
#       predict(model, new_data = test_df)
#
#       # 3. Horizon + override origin from tail of new_data
#       predict(model, new_data = test_df, horizon = 6L)
#
#       Returns a standardised tibble:
#         date | .value | .conf_lo | .conf_hi | .model_desc | .key
#       (.key = "prediction" for future, "actual" for test rows)
#
#   ③ modeltime-style calibration & accuracy
#       tf_calibrate(result, new_data)   — apply to test set
#       tf_accuracy(calibrated_result)   — RMSE/MAE/MAPE/SMAPE/RSQ
#       tf_forecast(result, h, new_data) — future/refit forecasts
#
# CV ENGINE (unchanged from v4)
#   forecast::tsCV() for all model types; tsCV returns error matrix
#   [T × h]; per-horizon metrics extracted via .tscv_metrics()
#
# MODELS
# ────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────
#   install.packages(c(
#     "torch","tidyverse","timetk","rsample","zoo",
#     "rBayesianOptimization","forecast","modeltime",
...
#
#   # Train
#   lstm_res  <- auto_lstm(train_df, horizon = 12L)
#   arima_res <- auto_arima_ts(train_df, horizon = 12L)
#
#   # New predict() API
#   predict(lstm_res, horizon = 12L)             # future forecast
#   predict(lstm_res, new_data = test_df)        # apply to test set
#   predict(arima_res, new_data = test_df, horizon = 6L)
#
#   # modeltime integration
#   mt <- tf_modeltime_table(lstm_res, arima_res)
#   cal <- modeltime_calibrate(mt, test_df)
#   modeltime_accuracy(cal)
#   modeltime_forecast(cal, new_data = future_df)
# ================================================================

suppressPackageStartupMessages({
  library(torch);     library(tidyverse); library(timetk)
  library(rsample);   library(zoo);       library(rBayesianOptimization)
  library(forecast);  library(modeltime); library(fracdiff)
  library(tseries);   library(rugarch);   library(patchwork)
  library(scales);    library(sandwich);  library(lmtest)
...
# Standard output columns for predict() / tf_forecast()
.pred_cols <- c("date",".value",".conf_lo",".conf_hi",".model_desc",".key")

#' Build a standardised prediction tibble
...
print.rnn_config <- function(x,...){
  ...
  cat("\n")
  cat(sprintf("  CV    : %s engine=forecast::tsCV [window=%s]\n",
    if(x$run_cv)"ON" else "OFF",
...
#' Rolling one-step-ahead OOS predictions on a test set
#' (used when new_data is provided to predict())
.rolling_oos <- function(model,train_series_sc,test_series_sc,
                          lags,scaler,device,is_nb=FALSE){
  all_sc  <- c(train_series_sc,test_series_sc)
  n_train <- length(train_series_sc); n_test <- length(test_series_sc)
...
.bo_optimize <- function(scaled_series,lags,arch,val_fraction,n_iter,init_points,
                          epochs,patience,acq,kappa,bounds,device,seed,
...
# Returns error matrix [T × h].
# ================================================================

# Safely coerce tsCV output to a matrix.
# When h = 1, forecast::tsCV() returns a plain numeric vector
# instead of a 1-column matrix, making ncol() return NULL and
# seq_len(NULL) throw an error.  This helper always returns a
# proper [T x h] matrix regardless of h.
.ensure_emat <- function(e_mat) {
  if (is.matrix(e_mat)) return(e_mat)
  # vector case (h = 1)
  matrix(as.numeric(e_mat), ncol = 1L,
         dimnames = list(names(e_mat), "h1"))
}

.tscv_metrics <- function(e_mat, y_actual){
  e_mat <- .ensure_emat(e_mat)
  h_max <- ncol(e_mat)
  dplyr::bind_rows(lapply(seq_len(h_max), function(h_step){
    e   <- e_mat[, h_step]; idx <- which(!is.na(e))
    if(length(idx) < 5L)
      return(tibble(h=h_step, RMSE=NA_real_, MAE=NA_real_, MAPE=NA_real_,
                    SMAPE=NA_real_, MBE=NA_real_, n=0L))
    e_h     <- e[idx]
    act_idx <- pmin(idx + h_step - 1L, length(y_actual))
    e_h     <- e_h[seq_along(act_idx)]
    a_h     <- y_actual[act_idx]; p_h <- a_h - e_h
    tibble(h     = h_step,
...
.tscv_oos_tibble <- function(e_mat, ts_y, dates_vec, strategy_name){
  e_mat <- .ensure_emat(e_mat)
  n     <- nrow(e_mat); h_max <- ncol(e_mat)
  dplyr::bind_rows(lapply(seq_len(h_max), function(h_step){
    e_col <- e_mat[, h_step]; idx <- which(!is.na(e_col))
    if(length(idx) == 0L) return(NULL)
    act_idx <- pmin(idx + h_step - 1L, n)
    a_v <- as.numeric(ts_y)[act_idx]; e_v <- e_col[idx]; p_v <- a_v - e_v
    tibble(fold=idx, h=h_step, date=dates_vec[act_idx],
           actual=a_v, predicted=p_v, residual=e_v, strategy=strategy_name)
  }))
}

.tscv_deep <- function(data,lags,horizon,hp,arch,cv_initial,cv_window,strategy,
                        epochs,patience,val_fraction,lr_factor,lr_patience,
                        grad_clip,scale,device,verbose,use_parallel=FALSE){
  t0 <- .tic(); arch_l <- tolower(arch); ts_y <- .df_to_ts(data)
  n_obs <- length(ts_y); dates <- data$date
  cat(sprintf("\n── tsCV [%s] | h=%d | initial=%s | window=%s | strategy=%s\n",
    toupper(arch_l),horizon,cv_initial%||%"auto",
    ifelse(is.null(cv_window),"expanding",cv_window),strategy))
  if(is.null(cv_initial)) cv_initial <- max(lags+horizon+5L,as.integer(n_obs*0.5))

  oos_list <- list(); e_mat_list <- list()

  for(strat in c("recursive","direct")){
    if(!strat%in%c(strategy,"both")&&strategy!="both") next
    if(strat=="direct"&&arch_l=="nbeats") next
    cat(sprintf("  tsCV [%s - %s]…\n",toupper(arch_l),strat))

    fc_fun <- local({
      lags_=lags; arch_l_=arch_l; hp_=hp; ep_=epochs; pat_=patience
      vf_=val_fraction; lrf_=lr_factor; lrp_=lr_patience; gc_=grad_clip
      ...
      function(y_train,h,...){
        ...
        if(s_=="recursive"){
          sv <- make_supervised(y_sc,lags_); n <- nrow(sv$X)
          nv <- floor(n*vf_); nt <- n-nv
          if(nt<5L) return(forecast::forecast(forecast::rwf(y_train),h=h))
          res <- train_deep_model(sv$X[1:nt,,drop=FALSE],sv$y[1:nt],
                   if(nv>0) sv$X[(nt+1):n,,drop=FALSE] else NULL,
                   if(nv>0) sv$y[(nt+1):n] else NULL,
          ...
          for(h_step in seq_len(h)){
            sv_h <- make_supervised_direct(y_sc,lags_,h_step); n_h <- nrow(sv_h$X)
            if(n_h<5L){fc_vals[h_step]<-NA_real_;next}
            nv_h <- floor(n_h*vf_); nt_h <- n_h-nv_h
            res_h <- train_deep_model(sv_h$X[1:nt_h,,drop=FALSE],sv_h$y[1:nt_h],
            ...
            fc_vals[h_step] <- sc_par$sd*predict_deep(res_h$model,X_pred,dev_)[1L]+sc_par$mean
          }
        }
        structure(list(mean=ts(fc_vals,start=n_tr+1L,frequency=frequency(y_train))),class="forecast")
      }
    })

    e_mat <- tryCatch(
      forecast::tsCV(y=ts_y,forecastfunction=fc_fun,h=horizon,
                     initial=cv_initial,window=cv_window),
      error=function(e){warning(sprintf("tsCV [%s %s] failed: %s",arch_l,strat,e$message));NULL})
  ...
  if(arch_l=="nbeats"&&strategy%in%c("recursive","both","direct")){
    ...
    fc_fun_nb <- local({
      ...
      lrf_=lr_factor; lrp_=lr_patience; gc_=grad_clip; sc_=scale; dev_=device; hp_=hp
      function(y_train,h,...){
        suppressPackageStartupMessages(library(torch))
        sc_par <- list(mean=mean(y_train,na.rm=TRUE),sd=sd(y_train,na.rm=TRUE))
        y_sc   <- if(sc_) (y_train-sc_par$mean)/sc_par$sd else y_train
        ...
        res <- train_deep_model(sv$X[1:nt,,drop=FALSE],sv$y[1:nt],
                 ...
                 num_layers=hp_$num_layers,dropout=hp_$dropout,dense_units=hp_$dense_units,
                 lr=hp_$lr,epochs=ep_,batch_size=hp_$batch_size,patience=pat_,
                 lr_factor=lrf_,lr_patience=lrp_,grad_clip=gc_,horizon=h,verbose=0L,device=dev_)
        fc_vals <- nbeats_forecast(res$model,tail(y_sc,lags_),h,sc_par,dev_)
        structure(list(mean=ts(fc_vals,start=length(y_train)+1L,frequency=frequency(y_train))),class="forecast")
      }
    })
    e_mat_nb <- tryCatch(
      forecast::tsCV(y=ts_y,forecastfunction=fc_fun_nb,h=horizon,
                     initial=cv_initial,window=cv_window),
      error=function(e){warning(sprintf("tsCV N-BEATS failed: %s",e$message));NULL})
    if(!is.null(e_mat_nb)){
      e_mat_nb <- .ensure_emat(e_mat_nb)
      e_mat_list[["nbeats"]] <- e_mat_nb
      oos_list[["nbeats"]]   <- .tscv_oos_tibble(e_mat_nb,ts_y,dates,"nbeats")
    }
  }

  if(length(e_mat_list)==0L) stop("tsCV produced no results.")

  by_h_list <- lapply(names(e_mat_list),function(s)
    .tscv_metrics(e_mat_list[[s]],as.numeric(ts_y))%>%mutate(strategy=s))
  by_horizon_df <- dplyr::bind_rows(by_h_list)

  overall <- dplyr::bind_rows(lapply(names(e_mat_list),function(s){
    bh <- by_horizon_df[by_horizon_df$strategy==s,]
    tibble(strategy=s,RMSE=mean(bh$RMSE,na.rm=TRUE),MAE=mean(bh$MAE,na.rm=TRUE),
           MAPE=mean(bh$MAPE,na.rm=TRUE),SMAPE=mean(bh$SMAPE,na.rm=TRUE),
           MBE=mean(bh$MBE,na.rm=TRUE),n_horizons=sum(!is.na(bh$RMSE)))
  }))

  cat(sprintf("\n── [%s] tsCV Summary ──\n",toupper(arch_l))); print(overall,n=Inf)
  list(cv_metrics=overall,by_horizon=by_horizon_df,oos_tbl=dplyr::bind_rows(oos_list),
       e_matrices=e_mat_list,elapsed_sec=.toc(t0))
}
...
.holdout_eval <- function(data,lags,horizon,hp,arch,holdout_frac,strategy,
                           epochs,patience,val_fraction,lr_factor,lr_patience,
                           grad_clip,scale,device,verbose){
  ...
  if(strategy%in%c("recursive","both")){
    res <- .train_tr()
    fv  <- .rolling_oos(res$model,tr_sc,tst_sc,lags,sc,device)
...
#' @export
auto_deep <- function(data,date_col="date",value_col="value",arch="lstm",horizon=12L,
                       strategy=c("both","recursive","direct"),device=NULL,config=NULL,
                       run_bo=NULL,bo_init=NULL,bo_iter=NULL,bo_epochs=NULL,bo_patience=NULL,
                       bo_bounds=NULL,final_epochs=NULL,final_patience=NULL,
                       run_cv=NULL,cv_initial=NULL,cv_window=NULL,cv_holdout_frac=NULL,
  ...
  cfg <- .resolve_cfg(config,list(
    ...
    final_patience=final_patience,run_cv=run_cv,cv_initial=cv_initial,cv_window=cv_window,
    cv_holdout_frac=cv_holdout_frac,val_fraction=val_fraction,lags=lags,scale=scale,
  ...
  cat(sprintf("  BO=%s | CV=%s%s\n",
    ...
    if(cfg$run_cv&&!is.null(cfg$cv_window)) sprintf("[window=%d]",cfg$cv_window)
    else if(cfg$run_cv) "[expanding]" else ""))
  ...
  def_hp  <- list(hidden_size=cfg$hidden_size,num_layers=cfg$num_layers,
                  dropout=cfg$dropout,dense_units=cfg$dense_units,lr=cfg$lr,batch_size=cfg$batch_size)
  runtime <- list()

  bo_result <- NULL
  if(cfg$run_bo){
    bounds  <- .resolve_bounds(cfg$bo_bounds)
    bo_out  <- .bo_optimize(scaled,lags_v,arch_l,cfg$val_fraction,cfg$bo_iter,cfg$bo_init,
                             cfg$bo_epochs,cfg$bo_patience,cfg$bo_acq,cfg$bo_kappa,
                             bounds,device,cfg$seed,horizon=horizon,use_parallel=use_par)
  ...
  if(cfg$run_cv){
    cv_out <- .tscv_deep(df,lags_v,horizon,best_hp,arch_l,cfg$cv_initial,cfg$cv_window,
                          strategy,cfg$final_epochs,cfg$final_patience,cfg$val_fraction,
                          cfg$lr_factor,cfg$lr_patience,cfg$grad_clip,cfg$scale,device,0L,
    ...
    ho_out <- .holdout_eval(df,lags_v,horizon,best_hp,arch_l,cfg$cv_holdout_frac,
                             strategy,cfg$final_epochs,cfg$final_patience,cfg$val_fraction,
                             cfg$lr_factor,cfg$lr_patience,cfg$grad_clip,cfg$scale,device,cfg$verbose)
  ...
  if(arch_l=="nbeats"){
    ...
    res_nb <- train_deep_model(X_tr,y_tr,X_vl,y_vl,lags=lags_v,arch="nbeats",
      ...
      lr_factor=cfg$lr_factor,grad_clip=cfg$grad_clip,horizon=horizon,
...
# ================================================================
# ██  PART 12 — AUTO-ARIMA (forecast + modeltime)
# ================================================================
...
auto_arima_ts <- function(data,date_col="date",value_col="value",
  ...
  if(verbose>0L) cat("\n── Auto-ARIMA ─────────────────────────────────────────────\n")
  fit_t0 <- .tic()
  # auto.arima() requires a logical for approximation, not NULL
  approx_arg <- if(is.null(approximation)) FALSE else as.logical(approximation)
  model  <- forecast::auto.arima(ts_obj,xreg=xreg,stepwise=stepwise,
    approximation=approx_arg,lambda=lambda,ic="aicc",trace=verbose>0L)
  fit_sec <- .toc(fit_t0)
  if(verbose>0L){
  ...
  if(run_cv){
    ...
    cat(sprintf("\n── ARIMA tsCV | h=%d | initial=%d | window=%s\n",
      horizon,cv_initial,ifelse(is.null(cv_window),"expanding",cv_window)))
    fc_fun_arima <- local({sw_=stepwise;ap_=if(is.null(approximation)) FALSE else as.logical(approximation);lm_=lambda
      function(y_tr,h,...){
        m_i <- tryCatch(forecast::auto.arima(y_tr,stepwise=sw_,approximation=ap_,
    ...
    e_mat <- tryCatch(forecast::tsCV(y=ts_obj,forecastfunction=fc_fun_arima,
                       h=horizon,initial=cv_initial,window=cv_window),
                      error=function(e){warning(sprintf("ARIMA tsCV: %s",e$message));NULL})
    if(!is.null(e_mat)){
      e_mat   <- .ensure_emat(e_mat)
      by_h    <- .tscv_metrics(e_mat,as.numeric(ts_obj))
      overall <- tibble(RMSE=mean(by_h$RMSE,na.rm=TRUE),MAE=mean(by_h$MAE,na.rm=TRUE),
        MAPE=mean(by_h$MAPE,na.rm=TRUE),SMAPE=mean(by_h$SMAPE,na.rm=TRUE),MBE=mean(by_h$MBE,na.rm=TRUE))
...
# ================================================================
# ██  PART 13 — AUTO-ETS (forecast + modeltime)
# ================================================================

#' @export
auto_ets_ts <- function(data,date_col="date",value_col="value",
                         horizon=12L,level=c(80,95),
                         model_spec="ZZZ",damped=NULL,
                         run_cv=TRUE,cv_initial=NULL,cv_window=NULL,
  ...
  fit_sec <- .toc(fit_t0)
  if(verbose>0L) cat(sprintf("  Selected: ETS(%s) | AICc=%.2f\n",model$method,model$aicc))
  freq_str  <- infer_frequency(df$date)
  fut_dates <- seq(max(df$date),by=freq_str,length.out=horizon+1L)[-1L]
  ...
  if(run_cv){
    ev_t0 <- .tic()
    if(is.null(cv_initial)) cv_initial <- max(as.integer(nrow(df)*0.5),2L*freq+horizon)
    cat(sprintf("\n── ETS tsCV | h=%d | initial=%d\n",horizon,cv_initial))
    fc_fun_ets <- local({ms_=model_spec;dm_=damped
      function(y_tr,h,...){
        m_i <- tryCatch(forecast::ets(y_tr,model=ms_,damped=dm_,ic="aicc"),error=function(e) NULL)
        if(is.null(m_i)) return(forecast::forecast(forecast::rwf(y_tr),h=h))
        forecast::forecast(m_i,h=h)
      }})
    e_mat <- tryCatch(forecast::tsCV(y=ts_obj,forecastfunction=fc_fun_ets,
                       h=horizon,initial=cv_initial,window=cv_window),
                      error=function(e){warning(sprintf("ETS tsCV: %s",e$message));NULL})
    if(!is.null(e_mat)){
      by_h    <- .tscv_metrics(e_mat,as.numeric(ts_obj))
      overall <- tibble(RMSE=mean(by_h$RMSE,na.rm=TRUE),MAE=mean(by_h$MAE,na.rm=TRUE),
        MAPE=mean(by_h$MAPE,na.rm=TRUE),SMAPE=mean(by_h$SMAPE,na.rm=TRUE),MBE=mean(by_h$MBE,na.rm=TRUE))
      oos_tbl <- .tscv_oos_tibble(e_mat,ts_obj,df$date,"ets")
      cv_results <- list(cv_metrics=overall,by_horizon=by_h,oos_tbl=oos_tbl,e_matrix=e_mat)
      if(verbose>0L){cat("\n── ETS tsCV ──\n");print(overall)}
    }
    eval_sec <- .toc(ev_t0)
    ...
    n_tst <- nrow(tst_df); fv <- numeric(n_tst)
    if(!is.null(m_ho))
      fv <- tryCatch(as.numeric(forecast::forecast(m_ho,h=n_tst)$mean),
                     error=function(e) rep(NA_real_,n_tst))
    else fv <- rep(NA_real_,n_tst)
    m_ho <- .safe_metrics(tst_df$value,fv)
...
# ================================================================
# ██  PART 14 — AUTO-ARFIMA, HAR-RV, GARCH (tsCV-based)
#   (condensed; same logic as v4 with tsCV engine)
# ================================================================
...
auto_arfima_ts <- function(data,date_col="date",value_col="value",
                            ...
                            cv_initial=NULL,cv_window=NULL,cv_holdout_frac=0.20,
                            seed=42L,verbose=1L){
  total_t0 <- .tic(); set.seed(seed)
  df <- data%>%rename(date=!!sym(date_col),value=!!sym(value_col))%>%
    arrange(date)%>%select(date,value)
  ...
  ts_obj <- .df_to_ts(df); freq <- .ts_freq(df$date)
  tryCatch({adf<-tseries::adf.test(ts_obj);kpss<-tseries::kpss.test(ts_obj)
  ...
  if(run_cv){
    ev_t0 <- .tic()
    if(is.null(cv_initial)) cv_initial <- max(as.integer(nrow(df)*0.5),2L*freq+horizon)
    fc_fun <- local({dr_=drange
      function(y_tr,h,...){
        m_i <- tryCatch(forecast::arfima(y_tr,drange=dr_,estim="mle"),error=function(e) NULL)
        if(is.null(m_i)) return(forecast::forecast(forecast::rwf(y_tr),h=h))
        tryCatch(forecast::forecast(m_i,h=h),
                 error=function(e) forecast::forecast(forecast::rwf(y_tr),h=h))
      }})
    e_mat <- tryCatch(forecast::tsCV(y=ts_obj,forecastfunction=fc_fun,
                       h=horizon,initial=cv_initial,window=cv_window),error=function(e) NULL)
    if(!is.null(e_mat)){
      by_h    <- .tscv_metrics(e_mat,as.numeric(ts_obj))
      overall <- tibble(RMSE=mean(by_h$RMSE,na.rm=TRUE),MAE=mean(by_h$MAE,na.rm=TRUE),
        MAPE=mean(by_h$MAPE,na.rm=TRUE),SMAPE=mean(by_h$SMAPE,na.rm=TRUE),MBE=mean(by_h$MBE,na.rm=TRUE))
...
auto_har_rv <- function(data,date_col="date",rv_col="value",horizon=1L,
                         ...
                         d_lags=1L,w_lags=5L,m_lags=22L,jump_col=NULL,rq_col=NULL,
                         log_transform=FALSE,run_cv=TRUE,cv_initial=NULL,cv_window=NULL,
                         cv_holdout_frac=0.20,seed=42L,verbose=1L){
  ...
  .bf <- function(rv){
    ...
    if(!is.null(jmp)&&variant%in%c("harj","harcj")){
      ...
      if(variant=="harj") X$J_d <- lag1_j else {X$RV_d<-NULL;X$C_d<-lag1_c;X$J_d<-lag1_j}
    }
    if(!is.null(rq)&&variant=="harq"){X$RQ_d<-c(rep(NA_real_,d_lags),sqrt(rq[seq_len(n-d_lags)]))}
    X
  }
  feats  <- .bf(rv_use)
  ...
  if(verbose>0L){cat(sprintf("\n── HAR-RV [%s] n=%d\n",variant,nrow(har_df)));print(summary(model))}
  freq_str  <- infer_frequency(df$date)
  fut_dates <- seq(max(df$date),by=freq_str,length.out=horizon+1L)[-1L]
  .har_fc <- function(mdl,rv_h,h,lt){
    ...
    for(step in seq_len(h)){
      ...
      lagw <- mean(rv_h[max(1,n_h-w_lags+1):n_h]); lagm <- mean(rv_h[max(1,n_h-m_lags+1):n_h])
  ...
  forecast_df <- tibble(date=fut_dates,forecast=.har_fc(model,tail(rv_use,max(m_lags,60L)),horizon,log_transform),
    lower_95=NA_real_,upper_95=NA_real_)
  cv_results <- NULL; holdout_results <- NULL; eval_sec <- 0
  if(run_cv){
    ev_t0 <- .tic(); ts_y <- ts(rv_use,frequency=.ts_freq(df$date))
    if(is.null(cv_initial)) cv_initial <- max(as.integer(length(rv_use)*0.5),m_lags+30L)
    fc_fun_har <- local({fml_=fml;d_=d_lags;w_=w_lags;m_=m_lags;lt_=log_transform
      function(y_tr,h,...){
        y_r  <- if(lt_) log(pmax(as.numeric(y_tr),1e-10)) else as.numeric(y_tr); n <- length(y_r)
        ...
        rv_h <- tail(y_r,max(m_,60L)); preds <- numeric(h)
        for(step in seq_len(h)){
          n_h <- length(rv_h)
          nd  <- data.frame(RV_d=rv_h[n_h],RV_w=mean(rv_h[max(1,n_h-w_+1):n_h]),
                             RV_m=mean(rv_h[max(1,n_h-m_+1):n_h]))
          p   <- predict(m_i,newdata=nd); preds[step] <- if(lt_) exp(p) else p
    ...
    e_mat <- tryCatch(forecast::tsCV(y=ts_y,forecastfunction=fc_fun_har,
                       h=horizon,initial=cv_initial,window=cv_window),error=function(e) NULL)
    if(!is.null(e_mat)){
    ...
    f_tr  <- .bf(rv_tr); df_tr <- na.omit(data.frame(y=rv_tr,f_tr))
...
auto_garch <- function(data,date_col="date",value_col="value",horizon=10L,
  ...
  cv_results <- NULL; holdout_results <- NULL; eval_sec <- 0
  if(run_cv){
    ev_t0 <- .tic(); ts_y <- ts(df$value,frequency=.ts_freq(df$date))
    if(is.null(cv_initial)) cv_initial <- max(as.integer(nrow(df)*0.5),100L)
    fc_fun_garch <- local({mt_=model_type;p_=p;q_=q;ao_=arma_order;dist_=distribution;mm_=mean_m
      function(y_tr,h,...){
        sp <- rugarch::ugarchspec(variance.model=list(model=mt_,garchOrder=c(p_,q_)),
          mean.model=list(armaOrder=ao_,include.mean=if(mm_=="zero") FALSE else TRUE),
          distribution.model=dist_)
        m_i <- tryCatch(rugarch::ugarchfit(sp,data=as.numeric(y_tr),solver="hybrid"),error=function(e) NULL)
        if(is.null(m_i)) return(forecast::forecast(forecast::rwf(y_tr),h=h))
        ...
        structure(list(mean=ts(mean_i,start=length(y_tr)+1L,frequency=frequency(y_tr))),class="forecast")
      }})
    e_mat <- tryCatch(forecast::tsCV(y=ts_y,forecastfunction=fc_fun_garch,
                       h=horizon,initial=cv_initial,window=cv_window),error=function(e) NULL)
    if(!is.null(e_mat)){
      by_h    <- .tscv_metrics(e_mat,as.numeric(ts_y))
      overall <- tibble(RMSE=mean(by_h$RMSE,na.rm=TRUE),MAE=mean(by_h$MAE,na.rm=TRUE),
...
# ================================================================
# ██  PART 15 — REDESIGNED predict() — THREE CALL SIGNATURES
# ================================================================
#
# For ALL model types, predict() now accepts:
#
#   Signature 1: predict(model, horizon = 12L)
#     → Future forecast h steps beyond the training series.
#       Returns .key = "prediction".
#
#   Signature 2: predict(model, new_data = test_df)
#     → Apply model to test_df rows (each row = one time step).
#       Uses rolling one-step-ahead prediction so each forecast
#       uses only true past values. Returns .key = "actual"
#       (terminology from modeltime: fitted values on new data).
#
#   Signature 3: predict(model, new_data = test_df, horizon = 6L)
#     → Re-anchor origin at tail of test_df, then forecast h
#       steps into the future. Returns .key = "prediction".
#
# ALL signatures return a standardised tibble:
#   date | .value | .conf_lo | .conf_hi | .model_desc | .key
#
# This is fully compatible with modeltime_calibrate() and
# modeltime_forecast() after wrapping in tf_modeltime_table().
# ================================================================
...
predict.auto_rnn <- function(object, horizon=NULL, new_data=NULL,
  ...
  # ── Signature 2: rolling OOS predictions on new_data ──────────
  if (!is.null(new_data) && is.null(horizon)) {
    ...
    test_vals  <- new_data[[val_col]]
    train_vals <- object$data$value
    res <- list()
    for (s in c("recursive","direct","nbeats")) {
      if (s == "nbeats" && !is_nb) next
      if (s != "nbeats" && (!s %in% c(strategy,"both")) && strategy != "both") next
      mdl_to_use <- if (s %in% c("recursive","nbeats")) object$model_recursive
                    else if (!is.null(object$direct_models)) object$direct_models[[1]]
                    else NULL
      if (is.null(mdl_to_use)) next
      tr_sc   <- scale_x(train_vals, sc)
      tst_sc  <- scale_x(test_vals,  sc)
      fv      <- .rolling_oos(mdl_to_use, tr_sc, tst_sc, object$lags, sc, dev)
      res_sd  <- sd(test_vals - fv, na.rm=TRUE)
      ci_w    <- qnorm(0.5 + level/200) * res_sd
      tbl <- .make_pred_tbl(
        date        = new_data$date,
        value       = fv,
        lo          = fv - ci_w,
        hi          = fv + ci_w,
        model_desc  = sprintf("%s [%s]", mdesc, s),
        key         = "actual"
      ) %>% mutate(.strategy = s)
      res[[s]] <- tbl
    }
    if (length(res) == 0L) stop("No model available for OOS prediction.")
  ...
  if (!is.null(new_data) && !is.null(horizon)) {
    ...
    res       <- .predict_future(object, last_win, h, fut_dates, sc, dev, strategy,
                                  level, mdesc, is_nb)
    return(res)
  }
...
# Internal helper: future forecast from a given last_win
.predict_future <- function(object, last_win, h, fut_dates, sc, dev,
                             strategy, level, mdesc, is_nb){
  ...
  if ((strategy %in% c("recursive","both") || is_nb) && !is.null(object$model_recursive)) {
    ...
    res[[strat_lbl]] <- .make_pred_tbl(
      fut_dates, fc, fc-ci_w, fc+ci_w,
      sprintf("%s [%s]", mdesc, strat_lbl), "prediction"
    ) %>% mutate(.strategy = strat_lbl)
  }
  if (strategy %in% c("direct","both") && !is_nb && !is.null(object$direct_models) &&
    ...
    res$direct <- .make_pred_tbl(
      fut_dates, fc, fc-ci_w, fc+ci_w,
      sprintf("%s [direct]", mdesc), "prediction"
    ) %>% mutate(.strategy = "direct")
  }
...
predict.auto_arima_ts <- function(object, horizon=NULL, new_data=NULL, level=NULL, xreg=NULL, ...){
  ...
  if (!is.null(new_data) && is.null(horizon)) {
    # Refit on all data through new_data, return 1-step-ahead filtered values
    ...
    all_ts <- .df_to_ts(all_df)
    n_tr   <- nrow(object$data)
    n_tst  <- nrow(new_data)
    refit  <- tryCatch(forecast::Arima(all_ts, model=object$model), error=function(e) NULL)
    if (!is.null(refit)) {
      af  <- as.numeric(fitted(refit))
      fv  <- af[(n_tr+1L):(n_tr+n_tst)]
    } else {
      fv  <- tryCatch(as.numeric(forecast::forecast(object$model, h=n_tst)$mean),
                      error=function(e) rep(NA_real_,n_tst))
    }
  ...
  base_model <- if (!is.null(new_data)) {
    tryCatch(forecast::Arima(.df_to_ts(new_data %>% rename_with(~c("date","value"),everything())),
                             model=object$model), error=function(e) object$model)
  } else object$model
  ...
  lo         <- as.numeric(fc$lower[, ncol(fc$lower)])
  hi         <- as.numeric(fc$upper[, ncol(fc$upper)])
  .make_pred_tbl(fut_dates, as.numeric(fc$mean), lo, hi, mdesc, "prediction")
}
...
predict.auto_ets_ts <- function(object, horizon=NULL, new_data=NULL, level=NULL, ...){
  ...
  if (!is.null(new_data) && is.null(horizon)) {
    stopifnot(is.data.frame(new_data))
    n_tst <- nrow(new_data); ts_nd  <- .df_to_ts(new_data %>% rename_with(~c("date","value"),everything()))
...
predict.auto_arfima_ts <- function(object, horizon=NULL, new_data=NULL, level=NULL, ...){
  ...
  if (!is.null(new_data) && is.null(horizon)) {
    stopifnot(is.data.frame(new_data))
    n_tst <- nrow(new_data); all_df <- dplyr::bind_rows(object$data %>% select(date,value),
      new_data%>%rename_with(~c("date","value"),everything()))
    all_ts <- .df_to_ts(all_df); n_tr <- nrow(object$data)
    refit  <- tryCatch(forecast::Arima(all_ts,model=object$model),error=function(e) NULL)
  ...
  if(is.null(fc)) return(object$forecast[seq_len(h),]%>%rename(.value=forecast,.conf_lo=lower_95,.conf_hi=upper_95)%>%mutate(.model_desc=mdesc,.key="prediction"))
  .make_pred_tbl(fut_dates,as.numeric(fc$mean),
                 as.numeric(fc$lower[,ncol(fc$lower)]),
                 as.numeric(fc$upper[,ncol(fc$upper)]),mdesc,"prediction")
}
...
predict.auto_har_rv <- function(object, horizon=NULL, new_data=NULL, ...){
  ...
  if (!is.null(new_data) && is.null(horizon)) {
    # Rolling OOS: use historical RV up to each test point
    rv_hist <- c(if(object$log_transform) log(pmax(object$data$value,1e-10)) else object$data$value)
    ...
    for(i in seq_len(n_tst)){
      ...
      n_c    <- length(rv_cur)
      nd     <- data.frame(RV_d=rv_cur[n_c],
                           RV_w=mean(rv_cur[max(1,n_c-object$w_lags+1):n_c]),
                           RV_m=mean(rv_cur[max(1,n_c-object$m_lags+1):n_c]))
      ...
      fv[i] <- if(object$log_transform) exp(p) else p
    }
    res_sd <- sd(fv - new_data[[2]], na.rm=TRUE) %||% sd(object$data$value)*0.1
  ...
  .har_fc <- function(rv_h,h){
    preds <- numeric(h)
    for(step in seq_len(h)){
      n_h <- length(rv_h); nd <- data.frame(RV_d=rv_h[n_h],
        RV_w=mean(rv_h[max(1,n_h-object$w_lags+1):n_h]),
        RV_m=mean(rv_h[max(1,n_h-object$m_lags+1):n_h]))
      p <- tryCatch(predict(object$model,newdata=nd),error=function(e) NA_real_)
...
predict.auto_garch <- function(object, horizon=NULL, new_data=NULL, ...){
  ...
  if (!is.null(new_data) && is.null(horizon)) {
    stopifnot(is.data.frame(new_data))
    # Refit on training+new_data, extract 1-step-ahead filtered predictions
    all_vals <- c(object$data$value, new_data[[setdiff(names(new_data),"date")[1]]])
    ...
    if (!is.null(refit)) {
      ...
      fv  <- rep(NA_real_,n_tst); sig <- rep(NA_real_,n_tst)
    }
  ...
  base_model <- if (!is.null(new_data)) {
    all_vals <- c(object$data$value, new_data[[setdiff(names(new_data),"date")[1]]])
    tryCatch(rugarch::ugarchfit(object$spec,data=all_vals,solver="hybrid"),
  ...
  mean_fc <- as.numeric(rugarch::fitted(fc)); sig_fc <- as.numeric(rugarch::sigma(fc))
  .make_pred_tbl(fut_dates,mean_fc,mean_fc-1.96*sig_fc,mean_fc+1.96*sig_fc,mdesc,"prediction")
}


# ================================================================
# ██  PART 16 — modeltime INTEGRATION
# ================================================================
#
# tf_modeltime_table() wraps any number of torchforecast result
# objects into a modeltime table that is compatible with the
# full modeltime workflow:
#   modeltime_calibrate()
#   modeltime_accuracy()
#   modeltime_forecast()
#   modeltime_refit()
#
# The table is built around modeltime::modeltime_table(), using
# custom parsnip/modeltime wrappers for each model type.
# For deep learning models a bridge parsnip spec is registered.
# ================================================================

#' Build a modeltime table from torchforecast result objects
#'
#' @param ...  Named or unnamed torchforecast result objects
#'             (auto_rnn, auto_arima_ts, auto_ets_ts, auto_arfima_ts,
#'              auto_har_rv, auto_garch).
#'
#' @return A modeltime table (tibble with .model_id, .model,
#'         .model_desc columns) compatible with the full modeltime
#'         calibrate → accuracy → forecast workflow.
#'
#' @examples
#' mt <- tf_modeltime_table(lstm_res, arima_res, ets_res)
#' cal <- modeltime_calibrate(mt, test_df)
#' modeltime_accuracy(cal)
#' modeltime_forecast(cal, new_data = future_df)
#'
...
tf_modeltime_table <- function(...) {
  ...
  # Build a list of modeltime-compatible parsnip model objects
  # by wrapping each result in a bridge workflow object
  model_list <- lapply(seq_along(results), function(i) {
    r   <- results[[i]]
    nm  <- nms[i]
    .tf_to_modeltime_bridge(r, nm)
  })

  modeltime::modeltime_table(!!!model_list)
}

# Internal: wrap a torchforecast object in a modeltime bridge
.tf_to_modeltime_bridge <- function(result_obj, label) {
  # We create a minimal workflow list that modeltime can call
  # predict() on. The bridge intercepts modeltime's calls and
  # routes them to our predict() methods.
  env    <- new.env(parent = emptyenv())
  env$tf_result <- result_obj
  env$label     <- label

  # Build a parsnip "null_model" workflow carrier that stores the
  # torchforecast object and overrides prediction
  bridge <- structure(
    list(
      .__tf_result  = result_obj,
      .__tf_label   = label,
      spec          = list(
        method      = list(pred = list(func = c(fun = "predict"))),
        cls         = "tf_bridge"
      ),
      fit           = list(
        lvl     = NULL,
        spec    = list(cls = "tf_bridge")
      )
    ),
    class = c("tf_bridge_workflow", "model_spec")
  )

  # modeltime wraps models in a tibble row internally
  # We use a special approach: create a modeltime_model_list entry
  wf <- workflows::workflow()
  wf <- workflows::add_model(wf, parsnip::null_model() %>% parsnip::set_engine("parsnip"))
  wf$.__tf_result <- result_obj
  wf$.__tf_label  <- label
  class(wf) <- c("tf_workflow", class(wf))
  wf
}

# ── modeltime predict dispatch for tf_workflow ──────────────────
#' @export
...
# ── tf_calibrate: calibrate on test data ─────────────────────────
#' Apply torchforecast models to test data (modeltime-style calibration)
#'
#' @param results  Named list of torchforecast result objects, OR
#'                 a single result object.
#' @param new_data Test data frame with date + value columns.
#'
#' @return Tibble with .model_id, .model_desc, .key,
#'         .actual, .prediction, .residuals and accuracy metrics.
#' @export
tf_calibrate <- function(results, new_data) {
  if (!is.list(results) || inherits(results, "auto_rnn") ||
      inherits(results, c("auto_arima_ts","auto_arfima_ts","auto_ets_ts",
                           "auto_har_rv","auto_garch"))) {
  ...
  val_col  <- setdiff(names(new_data), "date")[1]
  actuals  <- new_data[[val_col]]
  dates    <- new_data$date

  rows <- lapply(seq_along(results), function(i) {
    r   <- results[[i]]
    nm  <- names(results)[i] %||% sprintf("Model_%d", i)

    # For each strategy or single model, generate OOS predictions
    preds_tbl <- tryCatch(
      predict(r, new_data = new_data),
      error = function(e) {
        warning(sprintf("tf_calibrate [%s]: predict failed: %s", nm, e$message))
        NULL
      }
    )
    if (is.null(preds_tbl)) return(NULL)

    # One row per strategy (or single row if no strategy column)
    preds_tbl %>%
      mutate(.model_id   = i,
             .model_name = nm,
             .actual     = actuals,
             .residuals  = actuals - .value)
  })

  cal <- dplyr::bind_rows(Filter(Negate(is.null), rows))
  attr(cal, "new_data")   <- new_data
  attr(cal, "results")    <- results
  class(cal) <- c("tf_calibrated", class(cal))
  cal
}
...
#' Compute accuracy metrics from calibrated results (modeltime-style)
#'
#' @param calibrated  Object returned by tf_calibrate().
#' @return Tibble: .model_id, .model_name, .model_desc,
#'         MAE, MAPE, MASE, SMAPE, RMSE, RSQ.
#' @export
tf_accuracy <- function(calibrated) {
  if (!inherits(calibrated, "tf_calibrated"))
    stop("calibrated must be the output of tf_calibrate().")
...
#' Forecast from calibrated results (future or refit)
#'
#' @param calibrated  Object from tf_calibrate().
#' @param horizon     Steps ahead (overrides model default).
#' @param new_data    Optional new anchor data.
#'
#' @return Tidy tibble: .model_id, .model_desc, .key, .value,
#'         .conf_lo, .conf_hi, date, .model_name.
#' @export
tf_forecast <- function(calibrated, horizon=NULL, new_data=NULL) {
  ...
  rows <- lapply(seq_along(results), function(i) {
    ...
    nm <- names(results)[i] %||% sprintf("Model_%d",i)
    tryCatch(
      predict(r, horizon=horizon, new_data=new_data) %>%
  ...
  dplyr::bind_rows(Filter(Negate(is.null), rows)) %>%
    select(.model_id, .model_name, .model_desc, .key,
           date, .value, .conf_lo, .conf_hi, everything())
}

#' Convenience: combine actuals + predictions for plotting
#'
#' @param calibrated  tf_calibrated object.
#' @param horizon     Optional future horizon.
#' @return Tidy combined tibble for plot_modeltime_forecast().
#' @export
tf_forecast_combined <- function(calibrated, horizon=NULL, new_data=NULL) {
  actual_tbl <- attr(calibrated,"new_data") %>%
    mutate(.key="actual", .value=.data[[setdiff(names(.),"date")[1]]],
           .conf_lo=NA_real_,.conf_hi=NA_real_,.model_desc="Actual",.model_id=0L,.model_name="Actual") %>%
    select(.model_id,.model_name,.model_desc,.key,date,.value,.conf_lo,.conf_hi)

  fc_tbl <- tf_forecast(calibrated, horizon=horizon, new_data=new_data)

  dplyr::bind_rows(actual_tbl, fc_tbl)
}


# ================================================================
# ██  PART 17 — OOS HELPER (.get_oos / fitted_oos / predicted)
# ================================================================
...
#' @export
fitted_oos.auto_ets_ts    <- function(object,...) .get_oos(object)
#' @export
...
predicted.auto_rnn <- function(object,strategy=c("both","recursive","direct"),aggregate=FALSE,...){
  oos <- .get_oos(object,match.arg(strategy)); if(!aggregate) return(oos)
  has_s <- "strategy"%in%names(oos)
  oos%>%group_by(across(all_of(c("h",if(has_s)"strategy" else NULL))))%>%
    summarise(mean_actual=mean(actual),mean_predicted=mean(predicted),
              RMSE=sqrt(mean(residual^2)),MAE=mean(abs(residual)),.groups="drop")
...
#' @export
predicted.auto_arima_ts  <- .pred_default
#' @export
predicted.auto_ets_ts    <- .pred_default
#' @export
predicted.auto_arfima_ts <- .pred_default
#' @export
predicted.auto_har_rv    <- .pred_default
#' @export
predicted.auto_garch     <- .pred_default
...
print.auto_rnn <- function(x,digits=4L,n_fc=5L,...){
  ...
  cat(.hdr("predict() usage"),"\n")
  cat(sprintf("  predict(model, horizon = %d)          # future forecast\n",x$horizon))
  cat(sprintf("  predict(model, new_data = test_df)    # rolling OOS on test set\n"))
  cat(sprintf("  predict(model, new_data, horizon = h) # forecast from test tail\n"))
  ...
  if(length(fc_list)>0){
    cat(.hdr("Default forecast (use predict() for full control)"),"\n")
    for(fc in fc_list){ cat(sprintf("  [%s]\n",toupper(fc$strategy[1])))
      for(i in seq_len(min(n_fc,nrow(fc))))
        cat(sprintf("    %-12s .value=%s .conf_lo=%s .conf_hi=%s\n",format(fc$date[i]),
...
print.auto_arima_ts <- function(x,digits=4L,...){
  ...
  if(!is.null(x$cv_results)){cat("  tsCV overall:\n");print(x$cv_results$cv_metrics)}
  cat(sprintf("  total=%s\n",.fmt_elapsed(rt$total_sec)))
  cat(.box(),"\n\n"); invisible(x)
}

#' @export
print.auto_ets_ts <- function(x,digits=4L,...){
  cat("\n",.box(),"\n  Auto-ETS (torchforecast v5)\n",.box(),"\n",sep="")
  cat(sprintf("  ETS(%s) | AICc=%.2f\n",x$model$method,x$model$aicc))
  cat("  predict() signatures: horizon | new_data | (new_data + horizon)\n")
  if(!is.null(x$cv_results)){cat("  tsCV overall:\n");print(x$cv_results$cv_metrics)}
  cat(sprintf("  total=%s\n",.fmt_elapsed(x$runtime$total_sec)))
  cat(.box(),"\n\n"); invisible(x)
}

#' @export
print.auto_arfima_ts <- function(x,...){
  cat("\n",.box(),"\n  Auto-ARFIMA (torchforecast v5)\n",.box(),"\n",sep="")
  cat(sprintf("  d=%.6f | total=%s\n",x$d_hat,.fmt_elapsed(x$runtime$total_sec)))
  if(!is.null(x$cv_results)){cat("  tsCV overall:\n");print(x$cv_results$cv_metrics)}
  cat(.box(),"\n\n"); invisible(x)
}
...
print.auto_har_rv <- function(x,...){
  cat("\n",.box(),"\n  HAR-RV (torchforecast v5)\n",.box(),"\n",sep="")
  cat(sprintf("  variant=%s | lags: d=%d w=%d m=%d | total=%s\n",
              x$variant,x$d_lags,x$w_lags,x$m_lags,.fmt_elapsed(x$runtime$total_sec)))
  if(!is.null(x$cv_results)){cat("  tsCV overall:\n");print(x$cv_results$cv_metrics)}
  cat(.box(),"\n\n"); invisible(x)
}
...
print.auto_garch <- function(x,...){
  ...
  cat("\n",.box(),"\n  GARCH (torchforecast v5)\n",.box(),"\n",sep="")
  cat(sprintf("  %s(%d,%d) dist=%s | AIC=%.4f | total=%s\n",
              x$model_type,x$p,x$q,x$distribution,ic[1],.fmt_elapsed(rt$total_sec)))
  if(!is.null(x$cv_results)){cat("  tsCV overall:\n");print(x$cv_results$cv_metrics)}
  cat(.box(),"\n\n"); invisible(x)
}
...
cv_performance.auto_rnn <- function(object,strategy=c("both","recursive","direct"),by_horizon=TRUE,...){
  ...
  cat("\n── OOS CV Performance (forecast::tsCV) ──\n"); print(per_h,n=Inf)
...
cv_performance.auto_arima_ts  <- function(object,...){
  cat("\n── ARIMA tsCV ──\n")
  if(!is.null(object$cv_results)){print(object$cv_results$cv_metrics);print(object$cv_results$by_horizon,n=Inf)}
  invisible(object$cv_results)
}
#' @export
cv_performance.auto_ets_ts    <- cv_performance.auto_arima_ts
#' @export
...
.fc_panel <- function(hist_df,fc_all,label,colour,bt){
  p <- ggplot()+geom_line(data=hist_df,aes(date,value),colour=.PAL$actual,linewidth=0.8)+
...
plot.auto_rnn <- function(x,which=NULL,last_n=NULL,...){
  ...
  if(!is.null(x$history_recursive)&&nrow(x$history_recursive)>0){
    ...
    panels$training <- h%>%select(epoch,Training=train_loss,Validation=val_loss)%>%
      ...
      ggplot(aes(epoch,MSE,colour=split))+geom_vline(xintercept=best_ep,linetype="dotted",colour="grey60")+
      geom_line(linewidth=0.85)+scale_colour_manual(values=c(Training=.PAL$train,Validation=.PAL$val),name=NULL)+
      labs(title="Training Loss",x="Epoch",y="MSE")+.bt()
  }
  if(!is.null(x$cv_results)&&!is.null(x$cv_results$by_horizon)){
    bh <- x$cv_results$by_horizon; has_s <- "strategy"%in%names(bh)&&dplyr::n_distinct(bh$strategy)>1
    panels$cv_by_h <- bh%>%select(h,if(has_s)"strategy",RMSE,MAE)%>%
      ...
      labs(title="tsCV Metrics by Horizon",x="h",y="Error")+.bt()
  }
  fv <- tryCatch(fitted(x),error=function(e) NULL)
  if(!is.null(fv))
    panels$residuals <- ggplot(fv,aes(date,residual))+
      geom_hline(yintercept=0,colour="grey40")+
      geom_hline(yintercept=c(-2,2)*sd(fv$residual,na.rm=TRUE),linetype="dashed",colour=.PAL$direct,alpha=0.7)+
  ...
  pw  <- patchwork::wrap_plots(panels[sel],ncol=2L)+
    patchwork::plot_annotation(title=sprintf("Auto%s Dashboard (tsCV | v5)",mt),
...
.stat_plot <- function(x,label,colour){
  ...
  if(!is.null(x$cv_results)&&!is.null(x$cv_results$by_horizon))
    panels$by_h <- x$cv_results$by_horizon%>%select(h,RMSE,MAE)%>%
      ...
      labs(title="tsCV Error by Horizon",x="h",y="Error")+.bt()
  pw <- patchwork::wrap_plots(panels,ncol=2L)+
    patchwork::plot_annotation(title=sprintf("%s Dashboard (tsCV | v5)",label),
      theme=theme(plot.title=element_text(face="bold",size=14,hjust=0.5)))
  print(pw); invisible(pw)
}
#' @export
...
.oos_dash <- function(oos,hist_df,title,eval_label,model_label,ncol=2L){
  ...
  if(has_date){
    ...
    p1 <- p1+
      ...
      labs(title="Actual vs Fitted (OOS)",
           subtitle=sprintf("%s | black=actual, dashed=fitted",eval_label),x=NULL,y="Value")+bt
  ...
  if(has_h){
    ...
    panels$by_horizon <- ggplot(by_h,aes(h,value,colour=if(has_s) strategy else "model",linetype=metric))+
      ...
      labs(title=sprintf("Error by Horizon (tsCV — %d origins)",n_origins),x="h",y="Error")+bt
  }
...
plot_oos_fit.auto_rnn <- function(object,strategy=c("both","recursive","direct"),last_n=NULL,ncol=2L,title=NULL,...){
  ...
  if(is.null(title)) title <- sprintf("Auto%s — OOS Actual vs Fitted (v5)",toupper(object$model_type))
  ev <- if(!is.null(object$cv_results))"forecast::tsCV" else "Hold-out"
...
.make_oos_plot <- function(object,label,colour,last_n=NULL,ncol=2L,title=NULL,...){
  ...
  ev  <- if(!is.null(object$cv_results))"forecast::tsCV" else "Hold-out"
  .oos_dash(oos,hist_df,title%||%sprintf("%s — OOS Actual vs Fitted",label),ev,label,ncol)
}
#' @export
plot_oos_fit.auto_arima_ts  <- function(object,...){
  ord <- object$model$arma
  .make_oos_plot(object,sprintf("Auto-ARIMA(%d,%d,%d)",ord[1],ord[6],ord[2]),.PAL$arima,...)
}
#' @export
...
#' Plot tf_forecast / tf_calibrate output (modeltime-style)
#' @export
plot_tf_forecast <- function(combined_tbl, ...){
  has_s <- ".strategy"%in%names(combined_tbl)
  pal_m <- setNames(
    scales::hue_pal()(dplyr::n_distinct(combined_tbl$.model_desc)),
  ...
  p <- ggplot(combined_tbl,aes(date,.value,colour=.model_desc))+
    ...
    labs(title="torchforecast — Forecast (modeltime-style)",
...
dm_compare <- function(result1,result2,strategy1="recursive",strategy2="recursive",
  ...
  e1<-(oos1$actual-oos1[[pc(oos1)]])[seq_len(n_use)]; e2<-(oos2$actual-oos2[[pc(oos2)]])[seq_len(n_use)]
...
compare_all_models <- function(data,rnn_models=c("lstm","gru","rnn"),
                                ...
                                run_cv=TRUE,cv_holdout_frac=0.20,cv_initial=NULL,cv_window=NULL,
  ...
  train_model <- function(nm,dat){
    tryCatch(switch(nm,
      LSTM=auto_lstm(dat,horizon=horizon,run_cv=run_cv,cv_holdout_frac=cv_holdout_frac,
                     cv_initial=cv_initial,cv_window=cv_window,config=rnn_config_obj,use_parallel=FALSE,verbose=0L,...),
  ...
  .pull_m <- function(nm,res,metric){
    ...
    if(inherits(res,"auto_rnn")){
      ...
      if(!is.null(res$cv_results)&&!is.null(res$cv_results$cv_metrics)){
        ...
        if("strategy"%in%names(m)){r<-m[m$strategy==strat,,drop=FALSE];if(nrow(r)>0&&metric%in%names(r)) return(r[[metric]][1])}
      ...
      if(!is.null(res$cv_results)&&!is.null(res$cv_results$cv_metrics)){
        m <- res$cv_results$cv_metrics; if(metric%in%names(m)) return(m[[metric]][1])
      }
      if(!is.null(res$holdout_results)){
  ...
  comp_plot <- ggplot(plot_long,aes(model,value,fill=model))+
    ...
    labs(title="Model Comparison — OOS Metrics (forecast::tsCV)",
         subtitle=sprintf("Ranked by %s | total=%s | %d workers",rank_by,.fmt_elapsed(total_sec),.n_workers()),
         x=NULL,y="Value")+bt
  print(comp_plot)
  rt_plot <- ranking%>%filter(!is.na(runtime_sec))%>%mutate(model=factor(model,levels=model))%>%
    ...
    scale_fill_manual(values=model_pal,breaks=names(model_pal))+
    labs(title="Runtime",subtitle=sprintf("total=%s | workers=%d",.fmt_elapsed(total_sec),.n_workers()),
         x=NULL,y="Seconds")+bt
  print(rt_plot)
...
# ── Uncomment to run ─────────────────────────────────────────────────
#
# source("torchforecast.R"); library(zoo); library(timetk)
#
# df <- data.frame(
...
# )
# # Split
# train_df <- df[1:120, ]   # 10 years train
# test_df  <- df[121:144,]  # 2 years test
#
# # ── 1. Train ─────────────────────────────────────────────────────
# cfg  <- rnn_config(run_bo = TRUE, bo_iter = 10L, run_cv = TRUE, seed = 42L)
# lstm <- auto_lstm(train_df, horizon = 24L, config = cfg)
# arima_res <- auto_arima_ts(train_df, horizon = 24L, run_cv = TRUE)
# ets_res   <- auto_ets_ts(train_df,   horizon = 24L, run_cv = TRUE)
#
# # ── 2. New predict() API — three signatures ───────────────────────
# # Signature 1: future forecast h steps
# p1 <- predict(lstm, horizon = 24L)
# p1 <- predict(arima_res, horizon = 24L)
#
# # Signature 2: apply to test data (OOS fitted values vs actuals)
# p2 <- predict(lstm, new_data = test_df)           # rolling 1-step-ahead on test set
# p2 <- predict(arima_res, new_data = test_df)      # Kalman-filtered 1-step on test set
# p2 <- predict(ets_res, new_data = test_df)
#
# # Signature 3: horizon from tail of test data
# p3 <- predict(lstm, new_data = test_df, horizon = 12L)
# p3 <- predict(arima_res, new_data = test_df, horizon = 12L)
#
# # All return: date | .value | .conf_lo | .conf_hi | .model_desc | .key
#
# # ── 3. modeltime-style workflow ──────────────────────────────────
# mt  <- tf_modeltime_table(lstm = lstm, ARIMA = arima_res, ETS = ets_res)
# cal <- tf_calibrate(list(lstm=lstm, ARIMA=arima_res, ETS=ets_res), test_df)
# print(cal)                          # prints accuracy table
# tf_accuracy(cal)                    # RMSE/MAE/MAPE/SMAPE/RSQ
# fc  <- tf_forecast(cal, horizon=24L) # future forecast from all models
# combined <- tf_forecast_combined(cal, horizon=24L)
# plot_tf_forecast(combined)          # modeltime-style chart
#
# # ── 4. Direct modeltime calibration ──────────────────────────────
# # tf_modeltime_table() → modeltime_calibrate() → modeltime_accuracy()
# mtt <- tf_modeltime_table(lstm, arima_res)
# # (modeltime functions work directly if workflows package is loaded)
#
# # ── 5. Diagnostics ───────────────────────────────────────────────
# plot(lstm)                          # training + tsCV dashboard
# plot_oos_fit(lstm)                  # actual vs OOS fitted
# plot_oos_fit(arima_res)
# cv_performance(lstm)                # by-horizon metrics
#
# # ── 6. DM test (from tsCV errors) ────────────────────────────────
# dm_out <- dm_compare(lstm, arima_res)
...
#   horizon = 24L, run_cv = TRUE, cv_window = NULL
# )
# # Use predict() on best model with test data
# best_nm  <- comp$ranking$model[1]
# best_mdl <- comp$results[[best_nm]]
# predict(best_mdl, new_data = test_df)        # OOS fitted
# predict(best_mdl, horizon = 12L)             # future forecast
#
# # ── 8. Full pipeline example ──────────────────────────────────────
# results <- list(
#   LSTM  = auto_lstm(train_df, horizon=24L, run_cv=TRUE),
#   ARIMA = auto_arima_ts(train_df, horizon=24L, run_cv=TRUE),
#   ETS   = auto_ets_ts(train_df, horizon=24L, run_cv=TRUE)
# )
# # Calibrate on test set
# cal <- tf_calibrate(results, test_df)
# tf_accuracy(cal)
# # Future forecast
